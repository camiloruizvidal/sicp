...
class ficha_familiar extends PDFReport  
{
	public function render_datos_generales($datos_tarjeta_familiar)
	{
		...
		$this->Cell(8,5,"Fecha:",0,0,'L');$this->Cell(142,5,"{$datos_tarjeta_familiar['fecha_apertura']}",0,0,'L');$this->Cell(20,5,"Tarjeta familiar N",0,0,'L');$this->Cell(20,5,"{$datos_tarjeta_familiar['id_tarjeta_familiar']}",0,0,'L');//primer cero indica que no lleve borde*/
		...
		$this->Cell(23,5,"{$datos_tarjeta_familiar['fecha_registro']}",0,0,'L');$this->Cell(26,5,"{$datos_tarjeta_familiar['responsable']}",0,0,'L');$this->Cell(24,5,"{$datos_tarjeta_familiar['fecha_proxima_visita']}",0,0,'L');$this->Cell(13,5,"{$datos_tarjeta_familiar['id_zona']}",0,0,'L');$this->Cell(28,5,"{$datos_tarjeta_familiar['departamento']}",0,0,'L'); $this->Cell(31,5,"{$datos_tarjeta_familiar['municipio']}",0,0,'L');$this->Cell(28,5,"{$datos_tarjeta_familiar['corregimiento']}",0,0,'L');$this->Cell(25,5,"{$datos_tarjeta_familiar['vereda']}",0,0,'L');
        ...
        $this->Cell(24,5,"{$datos_tarjeta_familiar['ficha_sisben']}",0,0,'L');$this->Cell(19,5,"{$datos_tarjeta_familiar['nivel']}",0,0,'L');$this->Cell(23,5,"{$datos_tarjeta_familiar['puntaje']}",0,0,'L');$this->Cell(27,5,"{$datos_tarjeta_familiar['telefono']}",0,0,'L');$this->Cell(46,5,"{$datos_tarjeta_familiar['direccion']}",0,0,'L');$this->Cell(30,5,"{$datos_tarjeta_familiar['potabilidad']}",0,0,'L');$this->Cell(29,5,"{$datos_tarjeta_familiar['domicilio']}",0,0,'L');
         //fin nuevos datos
...
$descripcion=utf8_decode($datos_variables_tarjeta_familiar[$i]['descripcion']);
 $this->Cell(53,4,"{$descripcion}",0,0,'L');$this->Cell(10,4,"{$datos_variables_tarjeta_familiar[$i]['valor']}",0,0,'L'); 
         $this->Ln();                             
...
$descripcion=utf8_decode($datos_variables_tarjeta_familiar[$j]['descripcion']);
 $this->Cell(18,5," ",0,0,'L');$this->Cell(76,5,"{$descripcion}",1,0,'C');$this->Cell(90,5,"{$datos_variables_tarjeta_familiar[$j]['valor']}",1,0,'C'); 
         $this->Ln();                             
...
$descripcion=utf8_decode($datos_variables_tarjeta_familiar[$j]['descripcion']);
 ...
 if(($z%2)==0)
 {
 	$this->CellFitSpace(73,5,"{$descripcion}",1,0,'L');$this->Cell(10,5,"{$datos_variables_tarjeta_familiar[$j]['valor']}",1,0,'C');
 	$this->Ln(); 
 }
 else
 {
    $this->Cell(18,5," ",0,0,'L');$this->CellFitSpace(73,5,"{$descripcion}",1,0,'L');$this->Cell(10,5,"{$datos_variables_tarjeta_familiar[$j]['valor']}",1,0,'C'); 
...
public function render_miembros_asociados($datos_persona,$data_programas)//ahora recibe dos parametros
	{
			...
			$this->Cell(70,5,"",0,0,'L');$this->Cell(53,5,"Miembros Asociados",0,0,'C');
			$this->Ln(); 
			$this->Ln(); 
...
$this->Cell(40,4,"Nombres y Apellidos:",0,0,'L');$this->Cell(40,4,"{$nombres}",0,0,'L');
$this->Ln();
$this->Cell(40,4,"Numero documento:",0,0,'L');$this->Cell(40,4,"{$datos_persona[$i]['documento']}",0,0,'L');
$this->Ln();
$this->Cell(40,4,"Sexo:",0,0,'L');$this->Cell(40,4,"{$datos_persona[$i]['sexo']}",0,0,'L');
$this->Ln();
$this->Cell(40,4,"Fecha nacimiento:",0,0,'L');$this->Cell(40,4,"{$datos_persona[$i]['fecha_nacimiento']}",0,0,'L');
$this->Ln();
...
$this->Ln();
 ...
 $this->CellFitSpace(90,5,"{$descripcion}",0,0,'L');$this->Cell(10,5,"{$datos_persona[$i]['caracteristicas_ficha'][$j]['valor']}",0,0,'L'); 
         $this->Ln();    
...
$this->Ln();
$this->SetFont('Arial','B','12');
$this->Cell(50,4,"Programa",1,0,'L');$this->Cell(147,4,"Descripcion",1,0,'L');
...
function OrganizarDato($Dato,$variable)
{
	...
	{
		case '1':$value=array('descripcion'=>$variable['descripcion'],'valor'=>$Dato["value"]);break;
		case '4':$value=array('descripcion'=>$variable['descripcion'],'valor'=>tipe_4($Dato,$variable["list_values"]));break;
		case '9':
		...
		break;
		default : $value=array('descripcion'=>$variable['descripcion'],'valor'=>$Dato);	break;
	}
	return $value;
}
